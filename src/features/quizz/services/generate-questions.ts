...
const questionData = {
    open_ended: {
      ...
      template: (topic: string, nivel:string ) => `Generate a random ${nivel} open-ended questions about ${topic}`,
      fields: {
        question: "",
        answer: "",
      },
    ...
    mcq: {
      ...
      template: (topic: string, nivel: string) => `Generate a random ${nivel} multiple-choice questions about ${topic}.`,
      fields: {
        question: "",
  ...
  export async function generateQuestions(input: unknown) {
    const { amount, topic, type, nivelType } = getQuestionsSchema.parse(input);
  
    const { prompt, template, fields } = questionData[type];
    const templates = Array(amount).fill(template(topic, nivelType));
