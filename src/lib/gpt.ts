...
> {
  ...
  for (let i = 0; i < num_tries; i++) {
    let output_format_prompt: string = `\nYou must output only valid JSON, without any additional text.
The JSON format should match exactly this structure: ${JSON.stringify(output_format)}.
Ensure that:
...
5. Generate the response in the following valid JSON format:
[
  {
    "question": "...",
    "answer": "...",
...
`;
    ...
    try {
      ...
      res = res
        .replace(/(\w+):/g, '"$1":') // Adiciona aspas nos nomes das chaves
        .replace(/\n/g, '')
        .replace(/[`']/g, '');
